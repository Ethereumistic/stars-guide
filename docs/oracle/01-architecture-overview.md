...
- **Pipeline-gated birth context** (v2): Birth data is injected into the prompt only when a pipeline declares `needsBirthData: true` — the `birth_chart` and `synastry` pipelines do this; `generic_chat` and `journal_recall` do not. Depth is controlled by instructions, not data scope. The birth_chart pipeline injects a [CHART DATA UNAVAILABLE] block when there's no stored data, so the AI responds in chart-reading format and asks for data.
- **Cross-context mixing** (v2): Birth data, journal context, and timespace context can coexist in a prompt when their respective pipelines are active. Data gathering is driven by pipeline requirements — if ANY active pipeline needs it, it's gathered.
