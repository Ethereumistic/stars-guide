# 🌌 Authentication & User State Documentation
...
1.  **Backend (Convex & @convex-dev/auth)**: Handles the "Heavy Lifting" — OAuth handshakes, password hashing, session management, and the source-of-truth database.
...
### 🛡️ Auth Configuration (`convex/auth.ts`)
...
### 🧠 Zustand Store (`src/store/use-user-store.ts`)
...
### 🔄 UserSync (`src/components/providers/user-sync.tsx`)
...
*This system is designed for scale. Treat the `users` table as sacred—it is the compass for the entire stars.guide experience.*
