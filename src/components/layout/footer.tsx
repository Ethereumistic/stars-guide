...
const navItems = [
  ...
  // { title: "Learn", href: "/learn", icon: GiGiftOfKnowledge },
  ...
  { title: "Birth Chart", href: "/onboarding", icon: GiMazeCornea },
];
...
const socialLinks = [
  {
    icon: BsTwitterX,
    href: "https://twitter.com/starsguide",
    label: "Twitter",
  },
  { icon: BsTiktok, href: "https://tiktok.com/starsguide", label: "Tiktok" },
  {
    icon: Facebook,
    href: "https://github.com/Ethereumistic/stars-guide",
    label: "Facebook",
  },
  {
    icon: Instagram,
    href: "https://instagram.com/starsguide",
    label: "Instagram",
  },
];
      ...
      <div className="mx-auto max-w-[90rem] px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
        <div className="xl:grid xl:grid-cols-3 xl:gap-8">
          ...
          <div className="flex flex-col items-center text-center xl:items-start xl:text-left space-y-8">
            ...
            <p className="max-w-xs text-sm text-muted-foreground font-sans italic">
              Navigate your fate. The map is written above, and we are here to
              help you read it.
            </p>
          ...
          <div className="mt-16 flex flex-col gap-12 xl:col-span-2 xl:mt-0 xl:grid xl:grid-cols-2 xl:gap-8">
            <div className="grid grid-cols-2 gap-8 md:gap-8">
              <div className="">
                <h3 className="font-mono text-sm font-semibold uppercase tracking-wider text-foreground text-center xl:text-left ml-5 xl:ml-0 ">
                  Sanctuary
                </h3>
                <ul className="mt-4 space-y-4 flex flex-col items-center xl:items-start">
                  {navItems.map((item) => {
                    const Icon = item.icon;
                    const isBirthChart = item.title === "Birth Chart";
                    const href = isBirthChart
                      ? isAuthenticated
                        ? "/dashboard"
                        : "/onboarding"
                      : item.href;
                    ...
                    return (
                      <li key={item.title}>
                        <Link
                          href={href}
                          className={cn(
                            "text-sm text-muted-foreground hover:text-primary transition-colors duration-300 font-serif flex items-center group/footer overflow-hidden",
        ...
        <div className="mt-12 border-t border-primary/10 pt-8 flex flex-col md:flex-row items-center justify-between gap-6">
          <p className="text-xs text-muted-foreground font-sans">
            &copy; {new Date().getFullYear()} stars.guide. All rights reserved.
            Crafting destinies one star at a time.
          </p>
