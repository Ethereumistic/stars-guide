...
export function CaptureActionBar({
    ...
    return (
        <div className={cn("space-y-3", className)}>
            ...
            {/* Expandable sections */}
            {showVoice && (
                <div className="animate-in fade-in slide-in-from-top-2 duration-200 border-t border-white/[0.04] pt-3">
                    <label className="text-[10px] font-sans uppercase tracking-[0.15em] text-white/30 mb-2 block">
                        Voice
                    </label>
...
/** Small icon button for the action bar */
function ActionIconBtn({
    ...
    return (
        <button
            ...
            onClick={onClick}
            className={cn(
                "inline-flex items-center gap-1.5 rounded-lg border px-2.5 py-1.5 text-[10px] font-sans uppercase tracking-[0.08em] transition-all duration-200",
                active
                    ? "border-white/15 bg-white/[0.08] text-white/60"
                    : "border-white/[0.06] bg-transparent text-white/30 hover:bg-white/[0.06] hover:text-white/50"
            )}
            title={label}
        >
            {icon}
