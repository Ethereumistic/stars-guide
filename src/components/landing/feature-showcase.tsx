...
import { TbArrowRight, TbBolt, TbBrain } from "react-icons/tb";
...
const PILLARS = [
  ...
  },
  {
    ...
    ui: {
      ...
      titleColorClass:
        "text-primary drop-shadow-[0_0_15px_rgba(212,175,55,0.3)]",
  ...
  },
  {
    ...
    ui: {
      ...
      titleColorClass:
        "text-galactic drop-shadow-[0_0_15px_rgba(157,78,221,0.3)]",
...
function PillarCard({
  ...
  return (
    ...
    >
      ...
      {/* Main Card */}
      <div
        className={`relative py-8 px-8 flex flex-col h-full bg-background/85 backdrop-blur-2xl rounded-3xl z-0 overflow-hidden shadow-2xl transition-transform duration-1000 border ${ui.borderColor}`}
      >
        <AnimatePresence>
          {showGlare && (
            <motion.div
        ...
        <div className="flex-1 z-10 space-y-4">
          ...
          <div className="flex items-center gap-3">
            <div
              className={`${ui.iconColor} ${ui.iconGlowColor} ${ui.iconAnimate}`}
            >
              <Icon className="size-10" />
            </div>
            <h3
              className={`text-3xl font-serif font-bold text-nowrap ${ui.titleColorClass}`}
            >
              {pillar.title}
            </h3>
          </div>
          ...
          <ul className="space-y-4">
            {pillar.capabilities.map((cap) => (
              <li key={cap} className="flex items-start gap-4">
                <div
                  className={`mt-0.5 rounded-full p-1 border ${ui.checkBorder}`}
                >
    ...
    <section className="relative w-full overflow-hidden">
      ...
      >
        ...
        <p className="text-muted-foreground/70 font-sans mt-3 max-w-lg mx-auto text-sm md:text-base">
          Three interconnected experiences. One learns who you are, one
          translates the sky, one remembers everything.
        </p>
      </motion.div>
