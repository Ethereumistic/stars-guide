...
void main() {
  ...
  // Deliberate saccadic eye movement — 3 gazes across 7 seconds
  float saccadePeriod = 7.0 / 3.0;
  float t = uTime / saccadePeriod;
  float epoch = floor(t);
  float frac = fract(t);

  // Smooth deliberate transition (~470ms), then hold (~1.86s)
  float ease = smoothstep(0.0, 0.2, frac);

  // Pseudo-random gaze targets per epoch (prime-multiplied for decorrelation)
  vec2 prevGaze = vec2(
    sin(epoch * 7.23) * 0.55 + cos(epoch * 13.51) * 0.45,
    cos(epoch * 9.14) * 0.55 + sin(epoch * 11.37) * 0.45
  );
  vec2 nextGaze = vec2(
    sin((epoch + 1.0) * 7.23) * 0.55 + cos((epoch + 1.0) * 13.51) * 0.45,
    cos((epoch + 1.0) * 9.14) * 0.55 + sin((epoch + 1.0) * 11.37) * 0.45
  );

  vec2 pupilOffset = mix(prevGaze, nextGaze, ease) * 0.1;
  ...
  // Pupil — animated by procedural saccade offset
