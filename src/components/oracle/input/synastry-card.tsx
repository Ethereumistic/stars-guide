...
import { X, UserPlus, Loader2, AlertCircle, CalendarDays } from "lucide-react"
import { format } from "date-fns"
import type { IconType } from "react-icons"
import { IoHeart } from "react-icons/io5"
import { GiRing, GiShatteredHeart } from "react-icons/gi"
import { FaUserFriends, FaBaby, FaBriefcase, FaHandshake, FaGraduationCap } from "react-icons/fa"
...
const RELATIONSHIP_OPTIONS: { value: string; label: string; icon: IconType }[] = [
  { value: "romantic_partner", label: "Romantic Partner", icon: IoHeart },
  { value: "spouse", label: "Spouse", icon: GiRing },
  { value: "friend", label: "Friend", icon: FaUserFriends },
  { value: "parent", label: "Parent", icon: MdFamilyRestroom },
  { value: "child", label: "Child", icon: FaBaby },
  { value: "sibling", label: "Sibling", icon: FaPeopleArrows },
  { value: "coworker", label: "Coworker", icon: FaBriefcase },
  { value: "business_partner", label: "Business Partner", icon: FaHandshake },
  { value: "mentor", label: "Mentor / Teacher", icon: FaGraduationCap },
  { value: "crush", label: "Crush", icon: FaHeartPulse },
  { value: "ex", label: "Ex-Partner", icon: GiShatteredHeart },
]
...
// ── Chart Preview Card ───────────────────────────────────────────────────────
// Renders a chart card identical in structure and style to OracleChartPreview.
...
// ── Panel Card ───────────────────────────────────────────────────────────────
// Matches the visual card style of ChartPreviewCard for a symmetrical layout.
  ...
  return (
    <div className="flex justify-end">
      <div className="relative w-full">
        ...
        >
          {/* ═══════════════════════════════════════════════════════════════════
              ROW 1: Two chart cards side-by-side (or chart + import panel)
              ═══════════════════════════════════════════════════════════════════ */}

          {/* ── Column 1: Chart A (User's chart) — always visible ── */}
          <ChartPreviewCard
          ...
          {/* ── Column 2: Chart B or Import panel ── */}
          <AnimatePresence mode="wait">
            {hasChartB && synastryData?.chartB ? (
              <motion.div
                key="chartB"
                    ...
                    <AnimatePresence mode="wait">
                      {sourceTab === "custom" ? (
                        ...
                        >
                          ...
                          {/* Location — compact autocomplete, dropdown opens upward */}
                          <div className="space-y-1">
                            <label className="text-[10px] uppercase tracking-wider text-white/40 font-medium">
                              Birth city
                            </label>
          ...
          {/* ═══════════════════════════════════════════════════════════════════
              ROW 2: Relationship selector / Synastry indicator
              Spans both columns, shown when chart B exists
              ═══════════════════════════════════════════════════════════════════ */}
          <AnimatePresence>
            {hasChartB && (
              <motion.div
                      ...
                      >
                        ...
                        <div className="px-3 pb-3 space-y-1.5">
                          ...
                          <div className="grid grid-cols-6 gap-1.5">
                            {RELATIONSHIP_OPTIONS.slice(0, 6).map((opt) => (
                              <button
                                key={opt.value}
                                type="button"
                                onClick={() => {
                                  onSetRelationship(opt.value)
                                }}
                          ...
                          {/* Row 2: 5 options + custom input card */}
                          <div className="grid grid-cols-6 gap-1.5">
                            {RELATIONSHIP_OPTIONS.slice(6).map((opt) => (
                              <button
                                key={opt.value}
                                type="button"
                                onClick={() => {
                            ...
                            {/* Custom input as 6th card */}
                            <div className="relative rounded-lg border border-white/10 overflow-hidden flex items-center justify-center py-2 px-1">
                              <input
                                type="text"
                                value={customRelationship}
                                onChange={(e) => setCustomRelationship(e.target.value)}
                                placeholder="Custom"
                                className="w-full bg-transparent text-sm text-white placeholder:text-white/30 focus:outline-none text-center"
                                onKeyDown={(e) => {
                                  if (e.key === "Enter" && customRelationship.trim()) {
                                    onSetRelationship(customRelationship.trim())
                                  }
                                }}
                              />
                              {customRelationship.trim() && (
                                <button
                                  ...
                                  className="absolute right-0.5 top-1/2 -translate-y-1/2 size-4 rounded-full bg-galactic/30 text-galactic flex items-center justify-center hover:bg-galactic/50 transition-colors"
                    ...
                    {isReady && synastryData?.relationship && (
                      ...
                      >
                        ...
                        <span className="inline-flex items-center gap-1.5 text-[10px] text-galactic/80 uppercase tracking-wider">
                          ...
                          {(() => {
                            const found = RELATIONSHIP_OPTIONS.find(o => o.value === synastryData.relationship)
                            if (found) {
                              const RIcon = found.icon
                              return <><RIcon className="text-sm text-galactic" />{" "}{found.label}</>
                            }
                            return <>{synastryData.relationship}</>
                          })()}
                        </span>
                        <button
                          type="button"
