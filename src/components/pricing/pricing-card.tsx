      ...
      >
        ...
        <div className="flex-1 z-10 space-y-4">
          <div className=" flex items-center gap-3">
            ...
            <h3
              className={`text-3xl font-serif text-nowrap font-bold ${ui.titleColorClass}`}
            >
              {plan.name}
            </h3>
          </div>
          ...
          <ul className="space-y-4 p-2">
            {plan.features.map((feature, i) => (
              <li
                key={i}
                className={`flex items-start gap-4 ${!feature.included && "opacity-60"}`}
              >
                {feature.included ? (
                  <div
                ...
                <span
                  className={`text-sm tracking-wide ${feature.included ? "text-white/90" : "text-slate-500/50 line-through"}`}
                >
                  {feature.text}
                </span>
              </li>
        ...
        <div className="mt-8  z-10 transition-transform duration-300 group-hover:-translate-y-1">
          <Link
            href={plan.href}
            className={`w-full ${ui.buttonGlowColor} relative inline-flex group/btn items-center justify-center p-px mb-2 overflow-hidden text-sm font-medium rounded-xl`}
          >
            {isPopular ? (
              <Button
                size="xl"
                variant="default"
                className="w-full uppercase font-serif font-bold"
              >
                {plan.cta}
              </Button>
            ) : isPremium ? (
